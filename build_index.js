...
<head>
    ...
    <title>EduPulse | Next-Gen Student Management System</title>
    ...
    <style>
        /* ===== CSS (Copy from your original exactly – I'm keeping it short here but it's identical to yours) ===== */
        :root {
            --bg-base: #0a0d18; --bg-surface: #111728; --bg-card: rgba(18,24,43,0.75); --bg-card-hover: rgba(28,36,62,0.85); --bg-input: rgba(15,23,42,0.7);
            --border-subtle: rgba(255,255,255,0.08); --border-focus: #6366f1; --primary: #6366f1; --primary-gradient: linear-gradient(135deg, #6366f1, #8b5cf6, #d946ef);
            --primary-hover: #4f46e5; --secondary: #06b6d4; --success: #10b981; --warning: #f59e0b; --danger: #ef4444;
            ...
            --shadow-glow: 0 0 25px rgba(99,102,241,0.35); --transition-smooth: all 0.25s cubic-bezier(0.4,0,0.2,1);
        }
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .btn { display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 10px 18px; font-size: 0.875rem; font-weight: 600; border-radius: var(--radius-md); border: 1px solid transparent; cursor: pointer; transition: var(--transition-smooth); font-family: 'Inter', sans-serif; white-space: nowrap; text-decoration: none; }
        ...
        .form-group label { display: flex; align-items: center; gap: 6px; color: var(--text-muted); font-size: 0.8rem; font-weight: 600; margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.5px; }
        .form-group label i { color: var(--primary); }
        .form-control, .form-select { width: 100%; padding: 12px 16px; border-radius: var(--radius-md); border: 1px solid var(--border-subtle); background: var(--bg-input); color: var(--text-main); font-size: 0.925rem; font-family: 'Inter', sans-serif; outline: none; transition: var(--transition-smooth); backdrop-filter: blur(10px); }
        ...
        .auth-header h2 { font-size: 1.6rem; font-weight: 700; color: #fff; } .auth-header p { color: var(--text-muted); font-size: 0.875rem; margin-top: 4px; }
        .auth-footer { margin-top: 24px; text-align: center; font-size: 0.875rem; color: var(--text-muted); } .auth-footer a { color: #818cf8; font-weight: 600; text-decoration: none; cursor: pointer; transition: var(--transition-smooth); } .auth-footer a:hover { color: #c084fc; text-decoration: underline; }

        .role-badge { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 30px; font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.6px; }
        ...
        .status-badge { display: inline-flex; align-items: center; gap: 4px; padding: 3px 10px; border-radius: 20px; font-size: 0.725rem; font-weight: 600; }
        ...
        .nav-links a { display: flex; align-items: center; gap: 8px; color: var(--text-muted); text-decoration: none; padding: 8px 14px; border-radius: var(--radius-md); font-size: 0.825rem; font-weight: 600; transition: var(--transition-smooth); }
        ...
        .avatar-circle { width: 32px; height: 32px; border-radius: 50%; background: linear-gradient(135deg, #4f46e5, #06b6d4); color: #fff; display: flex; align-items: center; justify-content: center; font-size: 0.8rem; font-weight: 700; text-transform: uppercase; }
        .user-meta { display: flex; flex-direction: column; line-height: 1.2; } .user-meta .name { font-size: 0.825rem; font-weight: 600; color: #fff; max-width: 110px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        ...
        .drawer-overlay { display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.7); backdrop-filter: blur(8px); z-index: 1100; opacity: 0; transition: opacity 0.3s ease; } .drawer-overlay.open { display: block; opacity: 1; }
        .mobile-drawer { position: fixed; top: 0; left: -300px; width: 280px; height: 100%; background: var(--bg-surface); border-right: 1px solid var(--border-subtle); z-index: 1200; display: flex; flex-direction: column; transition: left 0.35s cubic-bezier(0.4,0,0.2,1); padding: 24px 18px; box-shadow: 10px 0 30px rgba(0,0,0,0.7); } .mobile-drawer.open { left: 0; }
        ...
        .page-header-info h2 { font-size: 1.45rem; font-weight: 700; color: #fff; display: flex; align-items: center; gap: 10px; } .page-header-info h2 i { color: var(--primary); } .page-header-info p { color: var(--text-muted); font-size: 0.85rem; margin-top: 2px; }
        ...
        .stat-card h3 { font-size: 1.9rem; font-weight: 800; color: #fff; margin-bottom: 4px; } .stat-card p { color: var(--text-muted); font-size: 0.85rem; font-weight: 500; }
        ...
        .search-box { position: relative; flex: 1; min-width: 240px; max-width: 400px; } .search-box i { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--text-subtle); font-size: 0.9rem; } .search-box input { width: 100%; padding: 10px 14px 10px 38px; border-radius: 30px; border: 1px solid var(--border-subtle); background: var(--bg-card); color: #fff; font-size: 0.875rem; outline: none; transition: var(--transition-smooth); } .search-box input:focus { border-color: var(--primary); box-shadow: 0 0 15px rgba(99,102,241,0.2); }
        ...
        .modern-table { width: 100%; border-collapse: collapse; text-align: left; font-size: 0.875rem; }
        .modern-table th { padding: 14px 18px; background: rgba(255,255,255,0.02); color: var(--text-muted); font-weight: 600; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.6px; border-bottom: 1px solid var(--border-subtle); white-space: nowrap; }
        ...
        .report-selection-card { background: var(--bg-card); border: 1px solid var(--border-subtle); border-radius: var(--radius-lg); padding: 22px; text-align: center; cursor: pointer; transition: var(--transition-smooth); } .report-selection-card:hover, .report-selection-card.active { border-color: var(--primary); background: var(--bg-card-hover); transform: translateY(-4px); box-shadow: var(--shadow-glow); } .report-selection-card i { font-size: 2.2rem; color: var(--primary); margin-bottom: 12px; } .report-selection-card h4 { font-size: 1rem; font-weight: 700; margin-bottom: 4px; } .report-selection-card p { color: var(--text-muted); font-size: 0.75rem; }

        .modal-backdrop { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(5,7,15,0.8); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); z-index: 2000; align-items: center; justify-content: center; padding: 20px; opacity: 0; transition: opacity 0.25s ease; } .modal-backdrop.show { display: flex; opacity: 1; }
        .modal-dialog { background: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: var(--radius-xl); max-width: 550px; width: 100%; max-height: 90vh; overflow-y: auto; padding: 30px; box-shadow: 0 25px 60px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.05); position: relative; transform: scale(0.92); transition: transform 0.25s cubic-bezier(0.4,0,0.2,1); } .modal-backdrop.show .modal-dialog { transform: scale(1); } .modal-dialog.modal-lg { max-width: 800px; }
        ...
        .att-label { cursor: pointer; padding: 4px 10px; border-radius: 20px; font-size: 0.75rem; font-weight: 600; border: 1px solid var(--border-subtle); background: rgba(255,255,255,0.03); color: var(--text-muted); transition: var(--transition-smooth); }
        ...
        .toast { pointer-events: auto; background: rgba(18,24,43,0.95); backdrop-filter: blur(15px); -webkit-backdrop-filter: blur(15px); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); padding: 14px 20px; color: #fff; font-size: 0.875rem; display: flex; align-items: center; gap: 12px; box-shadow: 0 15px 35px rgba(0,0,0,0.6); animation: toastSlide 0.3s cubic-bezier(0.4,0,0.2,1); max-width: 380px; }
...
<body>
    ...
    <div id="loginView" class="auth-wrapper">
        <div class="auth-card">
            <div class="auth-header">
                <div class="auth-logo-badge"><i class="fas fa-graduation-cap"></i></div>
            ...
            <form id="loginForm" onsubmit="handleLogin(event)">
                <div class="form-group">
                    ...
                    <input type="email" id="loginEmail" class="form-control" placeholder="name@school.edu" required />
                </div>
                <div class="form-group">
                    <div style="display:flex;justify-content:space-between;align-items:center;">
                        <label><i class="fas fa-lock"></i> Password</label>
                        <a href="#" onclick="openForgotPasswordModal()" style="font-size:0.75rem;color:#818cf8;text-decoration:none;margin-bottom:6px;">Forgot password?</a>
                    </div>
    ...
    <div id="registerView" class="auth-wrapper" style="display:none;">
        <div class="auth-card">
            <div class="auth-header">
                <div class="auth-logo-badge" style="background:linear-gradient(135deg, #06b6d4, #3b82f6);"><i class="fas fa-user-plus"></i></div>
                <h2>Create Account</h2>
                <p>Join the student management portal</p>
            </div>
            <form id="registerForm" onsubmit="handleRegister(event)">
                ...
                <div class="form-group">
                    <label><i class="fas fa-envelope"></i> Email Address</label>
                    <input type="email" id="regEmail" class="form-control" placeholder="alex@school.edu" required />
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select id="regRole" class="form-select" required>
                        ...
                        <option value="admin">Administrator (Demo / Setup)</option>
                    </select>
                </div>
    ...
    <div id="appContainer" style="display:none;">
            ...
            <div class="nav-container">
                ...
                <div class="nav-right">
                    <div class="user-profile-widget">
                        ...
                        <div class="user-meta">
                            <span class="name" id="navUserName">User Name</span>
                            <span id="navUserRoleBadge" class="role-badge student">Student</span>
                        </div>
                    </div>
        ...
        <div id="mobileDrawer" class="mobile-drawer">
            <div class="drawer-header">
                <div class="logo"><div class="logo-icon"><i class="fas fa-graduation-cap"></i></div><div>Edu<span>Pulse</span></div></div>
                <button class="drawer-close" onclick="toggleMobileDrawer(false)"><i class="fas fa-xmark"></i></button>
            </div>
            <div style="padding:10px 0 16px;border-bottom:1px solid var(--border-subtle);margin-bottom:12px;">
                <div style="display:flex;align-items:center;gap:10px;">
                    <div class="avatar-circle" id="drawerAvatar">U</div>
                    <div><div id="drawerUserName" style="font-weight:700;font-size:0.9rem;">User</div><span id="drawerUserRoleBadge" class="role-badge student" style="margin-top:4px;">Student</span></div>
                </div>
            </div>
            ...
            <div style="margin-top:auto;padding-top:16px;border-top:1px solid var(--border-subtle);">
                <button class="btn btn-danger" style="width:100%;" onclick="handleLogout()"><i class="fas fa-arrow-right-from-bracket"></i> Sign Out</button>
            </div>
        </div>
        ...
        <main class="main-wrapper">
            ...
            <div id="page-dashboard" class="page-content active">
                <div class="page-header-box">
                    <div class="page-header-info"><h2><i class="fas fa-gauge-high"></i> Overview Dashboard</h2><p>Welcome back, <span id="welcomeUserName" style="color:#fff;font-weight:600;">User</span>! Here is your school summary.</p></div>
                    <div class="header-actions">
                        <button class="btn btn-secondary btn-sm" onclick="refreshDashboard()"><i class="fas fa-arrows-rotate"></i> Refresh</button>
                        <button id="adminQuickSeedBtn" class="btn btn-primary btn-sm" style="display:none;" onclick="seedDemoData()"><i class="fas fa-wand-magic-sparkles"></i> Seed Sample Data</button>
                    </div>
                </div>
                <div class="stats-grid">
                    <div class="stat-card"><div class="stat-card-top"><div class="stat-icon indigo"><i class="fas fa-users"></i></div><span class="status-badge active"><i class="fas fa-chart-line"></i> Total</span></div><h3 id="statTotalStudents">0</h3><p>Enrolled Students</p></div>
                    <div class="stat-card"><div class="stat-card-top"><div class="stat-icon cyan"><i class="fas fa-book-open"></i></div><span class="status-badge info"><i class="fas fa-layer-group"></i> Active</span></div><h3 id="statTotalCourses">0</h3><p>Courses Available</p></div>
                    <div class="stat-card"><div class="stat-card-top"><div class="stat-icon emerald"><i class="fas fa-calendar-check"></i></div><span class="status-badge active"><i class="fas fa-clock"></i> Today</span></div><h3 id="statPresentToday">0</h3><p>Students Present</p></div>
                    <div class="stat-card"><div class="stat-card-top"><div class="stat-icon amber"><i class="fas fa-dollar-sign"></i></div><span class="status-badge pending"><i class="fas fa-receipt"></i> Revenue</span></div><h3 id="statTotalFees">$0</h3><p>Collected Fees</p></div>
                </div>
                <div id="quickActionsContainer" class="quick-actions-bar">
                    ...
                    <div class="quick-action-btn" onclick="openBatchAttendanceModal()"><i class="fas fa-clipboard-check"></i><div class="text"><strong>Mark Attendance</strong><span>Batch roll-call today</span></div></div>
                    <div class="quick-action-btn" onclick="openGradeModal()"><i class="fas fa-award"></i><div class="text"><strong>Record Grade</strong><span>Add exam / assignment</span></div></div>
                ...
                <div class="charts-row">
                    <div class="card-panel"><div class="card-panel-header"><h3><i class="fas fa-chart-area" style="color:var(--primary);"></i> Attendance & Enrollment Trends</h3></div><div class="chart-container"><canvas id="dashboardTrendChart"></canvas></div></div>
                    <div class="card-panel"><div class="card-panel-header"><h3><i class="fas fa-chart-pie" style="color:var(--secondary);"></i> Fee Status Breakdown</h3></div><div class="chart-container"><canvas id="dashboardFeeChart"></canvas></div></div>
                </div>
            </div>

            <!-- ===== USERS ===== -->
            <div id="page-users" class="page-content">
                <div class="page-header-box"><div class="page-header-info"><h2><i class="fas fa-user-shield"></i> User & Role Management</h2><p>Control system access, promote user roles.</p></div><div class="header-actions"><button class="btn btn-primary btn-sm" onclick="seedDemoData()"><i class="fas fa-database"></i> Generate Demo Data</button></div></div>
                <div class="table-filter-bar">
                    <div class="search-box"><i class="fas fa-search"></i><input type="text" id="userSearchInput" placeholder="Search..." oninput="filterUsersTable(this.value)" /></div>
                    <div><select id="userRoleFilter" class="form-select" style="width:auto;padding:8px 14px;" onchange="filterUsersByRole(this.value)"><option value="">All Roles</option><option value="admin">Admin</option><option value="teacher">Teacher</option><option value="student">Student</option><option value="parent">Parent</option></select></div>
                </div>
                <div class="table-card"><div class="table-responsive"><table class="modern-table"><thead><tr><th>#</th><th>User Name</th><th>Email</th><th>Current Role</th><th>Change Role</th><th>Actions</th></tr></thead><tbody id="usersTableBody"><tr><td colspan="6" style="text-align:center;padding:24px;color:var(--text-muted);">Loading users...</td></tr></tbody></table></div></div>
            </div>

            <!-- ===== STUDENTS ===== -->
            <div id="page-students" class="page-content">
                <div class="page-header-box"><div class="page-header-info"><h2><i class="fas fa-user-graduate"></i> Students Directory</h2><p>Manage student enrollments.</p></div><div class="header-actions"><button id="addStudentBtn" class="btn btn-primary" onclick="openStudentModal()"><i class="fas fa-plus"></i> Add Student</button></div></div>
                <div class="table-filter-bar">
                    <div class="search-box"><i class="fas fa-search"></i><input type="text" id="studentSearchInput" placeholder="Search..." oninput="filterStudentsTable(this.value)" /></div>
                    <div><select id="studentCourseFilter" class="form-select" style="width:auto;padding:8px 14px;" onchange="filterStudentsByCourse(this.value)"><option value="">All Courses</option></select></div>
                </div>
                <div class="table-card"><div class="table-responsive"><table class="modern-table"><thead><tr><th>#</th><th>Student</th><th>Email</th><th>Enrolled Course</th><th>Status</th><th>Actions</th></tr></thead><tbody id="studentsTableBody"><tr><td colspan="6" style="text-align:center;padding:24px;color:var(--text-muted);">Loading students...</td></tr></tbody></table></div></div>
            </div>

            <!-- ===== COURSES ===== -->
            <div id="page-courses" class="page-content">
                <div class="page-header-box"><div class="page-header-info"><h2><i class="fas fa-book"></i> Course Catalog</h2><p>Manage academic courses.</p></div><div class="header-actions"><button id="addCourseBtn" class="btn btn-primary" onclick="openCourseModal()"><i class="fas fa-plus"></i> Add Course</button></div></div>
                <div class="table-card"><div class="table-responsive"><table class="modern-table"><thead><tr><th>#</th><th>Course Title</th><th>Code</th><th>Credits</th><th>Instructor</th><th>Enrolled</th><th>Actions</th></tr></thead><tbody id="coursesTableBody"><tr><td colspan="7" style="text-align:center;padding:24px;color:var(--text-muted);">Loading courses...</td></tr></tbody></table></div></div>
            </div>

            <!-- ===== ATTENDANCE ===== -->
            <div id="page-attendance" class="page-content">
                <div class="page-header-box"><div class="page-header-info"><h2><i class="fas fa-clipboard-user"></i> Attendance Management</h2><p>Track student roll-calls.</p></div><div class="header-actions"><button id="markAttendanceBtn" class="btn btn-primary" onclick="openBatchAttendanceModal()"><i class="fas fa-check-double"></i> Batch Mark Attendance</button></div></div>
                <div class="table-filter-bar">
                    <div class="search-box"><i class="fas fa-search"></i><input type="text" id="attendanceSearchInput" placeholder="Filter..." oninput="filterAttendanceTable(this.value)" /></div>
                    <div><input type="date" id="attendanceDateFilter" class="form-control" style="width:auto;padding:8px 12px;" onchange="loadAttendance()" /></div>
                </div>
                <div class="table-card"><div class="table-responsive"><table class="modern-table"><thead><tr><th>#</th><th>Student Name</th><th>Date</th><th>Status</th><th>Course</th><th>Actions</th></tr></thead><tbody id="attendanceTableBody"><tr><td colspan="6" style="text-align:center;padding:24px;color:var(--text-muted);">Loading attendance...</td></tr></tbody></table></div></div>
            </div>

            <!-- ===== GRADES ===== -->
            <div id="page-grades" class="page-content">
                <div class="page-header-box"><div class="page-header-info"><h2><i class="fas fa-star-half-stroke"></i> Grades & Performance</h2><p>Record exam scores.</p></div><div class="header-actions"><button id="addGradeBtn" class="btn btn-primary" onclick="openGradeModal()"><i class="fas fa-plus"></i> Add Grade</button></div></div>
                <div class="table-card"><div class="table-responsive"><table class="modern-table"><thead><tr><th>#</th><th>Student</th><th>Course</th><th>Score</th><th>Letter</th><th>Remarks</th><th>Actions</th></tr></thead><tbody id="gradesTableBody"><tr><td colspan="7" style="text-align:center;padding:24px;color:var(--text-muted);">Loading grades...</td></tr></tbody></table></div></div>
            </div>

            <!-- ===== FEES ===== -->
            <div id="page-fees" class="page-content">
                <div class="page-header-box"><div class="page-header-info"><h2><i class="fas fa-wallet"></i> Fees & Financial Ledger</h2><p>Manage tuition invoices.</p></div><div class="header-actions"><button id="addFeeBtn" class="btn btn-primary" onclick="openFeeModal()"><i class="fas fa-plus"></i> Issue Fee</button></div></div>
                <div class="table-card"><div class="table-responsive"><table class="modern-table"><thead><tr><th>#</th><th>Student</th><th>Description</th><th>Amount</th><th>Due Date</th><th>Status</th><th>Actions</th></tr></thead><tbody id="feesTableBody"><tr><td colspan="7" style="text-align:center;padding:24px;color:var(--text-muted);">Loading fees...</td></tr></tbody></table></div></div>
            </div>

            <!-- ===== REPORTS ===== -->
            <div id="page-reports" class="page-content">
                <div class="page-header-box"><div class="page-header-info"><h2><i class="fas fa-chart-line"></i> Reports & Data Insights</h2><p>Generate academic summaries.</p></div><div class="header-actions"><button class="btn btn-secondary btn-sm" onclick="exportCurrentReportCSV()"><i class="fas fa-file-csv"></i> Export CSV</button><button class="btn btn-primary btn-sm" onclick="window.print()"><i class="fas fa-print"></i> Print</button></div></div>
                <div class="reports-cards-grid">
                    <div class="report-selection-card active" onclick="generateReport('students', this)"><i class="fas fa-users"></i><h4>Students Registry</h4><p>Enrollment directory</p></div>
                    <div class="report-selection-card" onclick="generateReport('attendance', this)"><i class="fas fa-calendar-check"></i><h4>Attendance Metrics</h4><p>Attendance rates</p></div>
                    <div class="report-selection-card" onclick="generateReport('grades', this)"><i class="fas fa-award"></i><h4>Academic Performance</h4><p>Grade distribution</p></div>
                    <div class="report-selection-card" onclick="generateReport('fees', this)"><i class="fas fa-receipt"></i><h4>Revenue & Fees</h4><p>Collection & arrears</p></div>
                </div>
    ...
    <script>
        ...
        const firebaseConfig = {
            apiKey: "AIzaSyAp2bLZ076qHxDTEIIb6jn122vz07EARws",
            authDomain: "student-management-syste-a2be1.firebaseapp.com",
            projectId: "student-management-syste-a2be1",
            storageBucket: "student-management-syste-a2be1.firebasestorage.app",
            messagingSenderId: "1023410958855",
            appId: "1:1023410958855:web:87ff395bdcccbfa04c70a2"
        };
        ...
        // NEW CACHE SETTINGS - Fixes the deprecation warning
        db.settings({
            cache: {
                kind: 'persistent',
        ...
        // Loaded flags to prevent re-fetching (makes it FAST!)
        let dataLoaded = {
            students: false,
            courses: false,
            attendance: false,
        ...
        function syncUserProfile(user) {
            const cachedData = localStorage.getItem('edu_user_' + user.uid);
            if (cachedData) {
                try {
                    const parsed = JSON.parse(cachedData);
                    currentUserName = parsed.name || user.displayName || user.email.split('@')[0];
                    currentUserRole = parsed.role || 'student';
                    updateUserDisplay();
                    renderNavigation();
                } catch (e) {}
            } else {
                currentUserName = user.displayName || user.email.split('@')[0];
                currentUserRole = localStorage.getItem('edu_last_reg_role') || 'student';
                updateUserDisplay();
                renderNavigation();
            }
            ...
            userProfileUnsubscribe = userDocRef.onSnapshot({ includeMetadataChanges: false }, (doc) => {
                if (doc.exists) {
                    const data = doc.data();
                    currentUserName = data.name || user.displayName || user.email.split('@')[0];
                    currentUserRole = data.role || 'student';
                    localStorage.setItem('edu_user_' + user.uid, JSON.stringify({ name: currentUserName, email: user.email, role: currentUserRole }));
                    updateUserDisplay();
                    renderNavigation();
                } else {
                    const initialRole = localStorage.getItem('edu_last_reg_role') || 'student';
                    const initialName = user.displayName || user.email.split('@')[0];
                    userDocRef.set({ name: initialName, email: user.email, role: initialRole, createdAt: firebase.firestore.FieldValue.serverTimestamp() })
                        .catch(err => console.warn('Profile creation:', err.message));
                ...
                console.warn('Profile listener offline:', err.message);
                updateUserDisplay();
            });
        }
        ...
        function updateUserDisplay() {
            ...
            roleBadges.forEach(badge => {
                if (badge) {
                    ...
                    let icon = currentUserRole === 'admin' ? 'fa-crown' : currentUserRole === 'teacher' ? 'fa-chalkboard-user' : currentUserRole === 'parent' ? 'fa-user-group' : 'fa-user-graduate';
        ...
        async function handleLogin(e) {
            ...
            try {
                ...
                if (err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password') msg = 'Invalid credentials.';
        ...
        async function handleRegister(e) {
            ...
            if (!name || !email || password.length < 6) { showToast('Fill all fields (password min 6).', 'error'); return; }
            try {
                localStorage.setItem('edu_last_reg_role', role);
                const cred = await auth.createUserWithEmailAndPassword(email, password);
                localStorage.setItem('edu_user_' + cred.user.uid, JSON.stringify({ name, email, role }));
                ...
                else if (err.code === 'auth/operation-not-allowed') msg = 'Email/Password sign-in disabled.';
                showToast(msg, 'error');
            }
        }
        ...
        function handleLogout() { auth.signOut().then(() => { showToast('Logged out.', 'info'); toggleMobileDrawer(false); }); }
        ...
        // =========================================================
        // DATA FETCHING WITH CACHING (FAST!)
        // =========================================================
        async function loadDashboard() {
            try {
                if (!dataLoaded.students) {
                    const snap = await db.collection('students').get().catch(() => ({ docs: [] }));
                    cachedStudents = snap.docs.map(d => ({ id: d.id, ...d.data() }));
                    dataLoaded.students = true;
                }
                if (!dataLoaded.courses) {
                    const snap = await db.collection('courses').get().catch(() => ({ docs: [] }));
                    cachedCourses = snap.docs.map(d => ({ id: d.id, ...d.data() }));
                    dataLoaded.courses = true;
                }
                if (!dataLoaded.fees) {
                    const snap = await db.collection('fees').get().catch(() => ({ docs: [] }));
                    cachedFees = snap.docs.map(d => ({ id: d.id, ...d.data() }));
                    dataLoaded.fees = true;
                }
                if (!dataLoaded.attendance) {
                    const snap = await db.collection('attendance').limit(100).get().catch(() => ({ docs: [] }));
        ...
        async function updateUserRole(id, role) {
            try { await db.collection('users').doc(id).update({ role, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }); showToast('Role updated!', 'success'); if(currentUser && currentUser.uid === id) { currentUserRole = role; updateUserDisplay(); renderNavigation(); } loadUsers(true); } catch(e) { showToast(e.message, 'error'); }
        }
        async function deleteUserRecord(id) {
            if (!confirm('Delete this user?')) return;
            try { await db.collection('users').doc(id).delete(); showToast('User deleted.', 'success'); loadUsers(true); } catch(e) { showToast(e.message, 'error'); }
        }
        function filterUsersTable(q) { const query = q.toLowerCase(); renderUsersTable(cachedUsers.filter(u => (u.name && u.name.toLowerCase().includes(query)) || (u.email && u.email.toLowerCase().includes(query)) || (u.role && u.role.toLowerCase().includes(query)))); }
        function filterUsersByRole(role) { if(!role) renderUsersTable(cachedUsers); else renderUsersTable(cachedUsers.filter(u => u.role === role)); }
        ...
        async function loadStudents(force = false) {
            ...
            try {
                let snap;
                try { snap = await db.collection('students').orderBy('name').get({ source: 'cache' }); if(snap.empty) throw new Error('cache'); }
                catch(e) { snap = await db.collection('students').orderBy('name').get(); }
                cachedStudents = snap.docs.map(d => ({ id: d.id, ...d.data() }));
                dataLoaded.students = true;
        ...
        async function saveStudent(e) {
            ...
            const data = { name: document.getElementById('studentName').value.trim(), email: document.getElementById('studentEmail').value.trim(), course: document.getElementById('studentCourseSelect').value, status: document.getElementById('studentStatusSelect').value, updatedAt: firebase.firestore.FieldValue.serverTimestamp() };
            try {
                if (id) { await db.collection('students').doc(id).update(data); showToast('Updated.', 'success'); }
                else { data.createdAt = firebase.firestore.FieldValue.serverTimestamp(); await db.collection('students').add(data); showToast('Added!', 'success'); }
                closeModal('studentModal'); dataLoaded.students = false; loadStudents(true);
            } catch(e) { showToast(e.message, 'error'); }
        }
        function editStudent(id) { const s = cachedStudents.find(st => st.id === id); if(s) openStudentModal(s); }
        async function deleteStudent(id) {
            if (!confirm('Delete?')) return;
            try { await db.collection('students').doc(id).delete(); showToast('Deleted.', 'success'); dataLoaded.students = false; loadStudents(true); } catch(e) { showToast(e.message, 'error'); }
        }
        function filterStudentsTable(q) { const query = q.toLowerCase(); renderStudentsTable(cachedStudents.filter(s => (s.name && s.name.toLowerCase().includes(query)) || (s.email && s.email.toLowerCase().includes(query)) || (s.course && s.course.toLowerCase().includes(query)))); }
        function filterStudentsByCourse(course) { if(!course) renderStudentsTable(cachedStudents); else renderStudentsTable(cachedStudents.filter(s => s.course === course)); }
        ...
        async function loadCourses(force = false) {
            ...
            try {
                let snap;
                try { snap = await db.collection('courses').orderBy('name').get({ source: 'cache' }); if(snap.empty) throw new Error('cache'); }
                catch(e) { snap = await db.collection('courses').orderBy('name').get(); }
                cachedCourses = snap.docs.map(d => ({ id: d.id, ...d.data() }));
                dataLoaded.courses = true;
        ...
        async function saveCourse(e) {
            ...
            const data = { name: document.getElementById('courseName').value.trim(), code: document.getElementById('courseCode').value.trim(), credits: parseInt(document.getElementById('courseCredits').value)||3, instructor: document.getElementById('courseInstructor').value.trim(), updatedAt: firebase.firestore.FieldValue.serverTimestamp() };
            try {
                if (id) { await db.collection('courses').doc(id).update(data); showToast('Updated.', 'success'); }
                else { data.createdAt = firebase.firestore.FieldValue.serverTimestamp(); await db.collection('courses').add(data); showToast('Added!', 'success'); }
                closeModal('courseModal'); dataLoaded.courses = false; loadCourses(true);
            } catch(e) { showToast(e.message, 'error'); }
        }
        function editCourse(id) { const c = cachedCourses.find(cr => cr.id === id); if(c) openCourseModal(c); }
        async function deleteCourse(id) {
            if (!confirm('Delete?')) return;
            try { await db.collection('courses').doc(id).delete(); showToast('Deleted.', 'success'); dataLoaded.courses = false; loadCourses(true); } catch(e) { showToast(e.message, 'error'); }
        }
        ...
        async function loadAttendance(force = false) {
            ...
            try {
                let query = db.collection('attendance').orderBy('date', 'desc').limit(50);
                if (dateFilter) query = db.collection('attendance').where('date', '==', dateFilter);
        ...
        function renderAttendanceTable(records) {
            ...
            tbody.innerHTML = records.map((a, i) => \`
                <tr>
                    ...
                    <td>\${escapeHtml(a.courseName||'General')}</td>
        ...
        async function submitBatchAttendance() {
            ...
            items.forEach(item => {
                ...
                batch.set(ref, { studentId: sid, studentName: name, courseName: course, date, status, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
                count++;
            });
            try { await batch.commit(); showToast('Saved ' + count + ' records!', 'success'); closeModal('batchAttendanceModal'); dataLoaded.attendance = false; loadAttendance(true); loadDashboard(); } catch(e) { showToast(e.message, 'error'); }
        }
        async function deleteAttendance(id) {
            if (!confirm('Delete?')) return;
            try { await db.collection('attendance').doc(id).delete(); showToast('Deleted.', 'success'); dataLoaded.attendance = false; loadAttendance(true); } catch(e) { showToast(e.message, 'error'); }
        }
        function filterAttendanceTable(q) {
            const query = q.toLowerCase();
            renderAttendanceTable(cachedAttendance.filter(a => (a.studentName && a.studentName.toLowerCase().includes(query)) || (a.status && a.status.toLowerCase().includes(query)) || (a.courseName && a.courseName.toLowerCase().includes(query))));
        }
        ...
        async function loadGrades(force = false) {
            ...
            try {
                let snap;
                try { snap = await db.collection('grades').orderBy('studentName').get({ source: 'cache' }); if(snap.empty) throw new Error('cache'); }
                catch(e) { snap = await db.collection('grades').orderBy('studentName').get(); }
                cachedGrades = snap.docs.map(d => ({ id: d.id, ...d.data() }));
            ...
            tbody.innerHTML = grades.map((g, i) => {
                ...
                return \`
                    <tr>
                        ...
                        <td><span style="color:#38bdf8;font-weight:600;">\${escapeHtml(g.courseName)}</span></td>
        ...
        async function openGradeModal(grade = null) {
            await loadStudents(true); await loadCourses(true);
            ...
            document.getElementById('gradeStudentSelect').value = grade ? grade.studentName : '';
            document.getElementById('gradeCourseSelect').value = grade ? grade.courseName : '';
        ...
        async function saveGrade(e) {
            ...
            const data = { studentName: document.getElementById('gradeStudentSelect').value, courseName: document.getElementById('gradeCourseSelect').value, grade: parseInt(document.getElementById('gradeValue').value)||0, remarks: document.getElementById('gradeRemarks').value.trim(), updatedAt: firebase.firestore.FieldValue.serverTimestamp() };
            try {
                if (id) { await db.collection('grades').doc(id).update(data); showToast('Updated.', 'success'); }
                else { data.createdAt = firebase.firestore.FieldValue.serverTimestamp(); await db.collection('grades').add(data); showToast('Added!', 'success'); }
                closeModal('gradeModal'); dataLoaded.grades = false; loadGrades(true);
            } catch(e) { showToast(e.message, 'error'); }
        }
        function editGrade(id) { const g = cachedGrades.find(gr => gr.id === id); if(g) openGradeModal(g); }
        async function deleteGrade(id) {
            if (!confirm('Delete?')) return;
            try { await db.collection('grades').doc(id).delete(); showToast('Deleted.', 'success'); dataLoaded.grades = false; loadGrades(true); } catch(e) { showToast(e.message, 'error'); }
        }
        ...
        async function loadFees(force = false) {
            ...
            try {
                let snap;
                try { snap = await db.collection('fees').orderBy('dueDate', 'desc').get({ source: 'cache' }); if(snap.empty) throw new Error('cache'); }
                catch(e) { snap = await db.collection('fees').orderBy('dueDate', 'desc').get(); }
                cachedFees = snap.docs.map(d => ({ id: d.id, ...d.data() }));
                dataLoaded.fees = true;
        ...
        function renderFeesTable(fees) {
            ...
            tbody.innerHTML = fees.map((f, i) => \`
                <tr>
                    ...
                    <td><i class="fas fa-clock" style="color:var(--warning);margin-right:4px;"></i>\${f.dueDate||'N/A'}</td>
        ...
        async function openFeeModal(fee = null) {
            ...
            document.getElementById('feeStudentSelect').value = fee ? fee.studentName : '';
            ...
            document.getElementById('feeDueDate').value = fee ? fee.dueDate : new Date().toISOString().split('T')[0];
        ...
        async function saveFee(e) {
            ...
            const data = { studentName: document.getElementById('feeStudentSelect').value, description: document.getElementById('feeDescription').value.trim(), amount: parseFloat(document.getElementById('feeAmount').value)||0, dueDate: document.getElementById('feeDueDate').value, status: document.getElementById('feeStatusSelect').value, updatedAt: firebase.firestore.FieldValue.serverTimestamp() };
            try {
                if (id) { await db.collection('fees').doc(id).update(data); showToast('Updated.', 'success'); }
                else { data.createdAt = firebase.firestore.FieldValue.serverTimestamp(); await db.collection('fees').add(data); showToast('Issued!', 'success'); }
                closeModal('feeModal'); dataLoaded.fees = false; loadFees(true); loadDashboard();
            } catch(e) { showToast(e.message, 'error'); }
        }
        function editFee(id) { const f = cachedFees.find(fe => fe.id === id); if(f) openFeeModal(f); }
        async function deleteFee(id) {
            if (!confirm('Delete?')) return;
            try { await db.collection('fees').doc(id).delete(); showToast('Deleted.', 'success'); dataLoaded.fees = false; loadFees(true); loadDashboard(); } catch(e) { showToast(e.message, 'error'); }
        }
        function viewInvoice(id) {
            ...
            document.getElementById('invoiceDetailsBody').innerHTML = \`
                ...
                <div style="display:flex;justify-content:space-between;margin-bottom:10px;font-size:0.875rem;"><span style="color:var(--text-muted);">Student:</span><strong>\${escapeHtml(fee.studentName)}</strong></div>
                ...
                <div style="display:flex;justify-content:space-between;margin-bottom:10px;font-size:0.875rem;"><span style="color:var(--text-muted);">Due:</span><span>\${fee.dueDate}</span></div>
        ...
        async function generateReport(type, element = null) {
            ...
            try {
                if (type === 'students') {
                    ...
                    const snap = await db.collection('attendance').orderBy('date','desc').limit(100).get();
                    const list = snap.docs.map(d => d.data());
                    ...
                    container.innerHTML = \`
                        ...
                        <div class="table-responsive"><table class="modern-table"><thead><tr><th>Student</th><th>Date</th><th>Status</th></tr></thead><tbody>\${list.slice(0,20).map(a => \`<tr><td>\${escapeHtml(a.studentName)}</td><td>\${a.date}</td><td><span class="status-badge \${a.status}">\${capitalize(a.status)}</span></td></tr>\`).join('')}</tbody></table></div>
                    \`;
                } else if (type === 'grades') {
                    const snap = await db.collection('grades').orderBy('studentName').get();
                    const list = snap.docs.map(d => d.data());
                    ...
                    container.innerHTML = \`
                        ...
                        <div class="table-responsive"><table class="modern-table"><thead><tr><th>Student</th><th>Course</th><th>Grade</th><th>Letter</th></tr></thead><tbody>\${list.map(g => { const l=calcLetter(g.grade||0); return \`<tr><td>\${escapeHtml(g.studentName)}</td><td>\${escapeHtml(g.courseName)}</td><td><strong>\${g.grade}%</strong></td><td><span class="status-badge \${l.badge}">\${l.letter}</span></td></tr>\`; }).join('')}</tbody></table></div>
                    \`;
                } else if (type === 'fees') {
        ...
        async function seedDemoData() {
            ...
            try {
                const courses = [
                    { name: 'Computer Science 101', code: 'CS-101', credits: 4, instructor: 'Dr. Alan Turing' },
                    { name: 'Advanced Mathematics', code: 'MATH-201', credits: 3, instructor: 'Prof. Katherine Johnson' },
                    { name: 'Modern Physics', code: 'PHYS-102', credits: 4, instructor: 'Dr. Richard Feynman' },
                    { name: 'Digital Media & Design', code: 'ART-105', credits: 3, instructor: 'Prof. Paula Scher' }
                ];
                const students = [
                    { name: 'Emma Watson', email: 'emma@school.edu', course: 'Computer Science 101', status: 'active' },
                    { name: 'Liam Hemsworth', email: 'liam@school.edu', course: 'Advanced Mathematics', status: 'active' },
                    { name: 'Sophia Chen', email: 'sophia@school.edu', course: 'Modern Physics', status: 'active' },
                    { name: 'Noah Davis', email: 'noah@school.edu', course: 'Digital Media & Design', status: 'active' },
                    { name: 'Olivia Taylor', email: 'olivia@school.edu', course: 'Computer Science 101', status: 'active' }
                ];
                const today = new Date().toISOString().split('T')[0];
                const grades = [
                    { studentName: 'Emma Watson', courseName: 'Computer Science 101', grade: 94, remarks: 'Excellent' },
                    { studentName: 'Liam Hemsworth', courseName: 'Advanced Mathematics', grade: 82, remarks: 'Good' },
                    { studentName: 'Sophia Chen', courseName: 'Modern Physics', grade: 91, remarks: 'Top lab' },
                    { studentName: 'Noah Davis', courseName: 'Digital Media & Design', grade: 76, remarks: 'Creative' }
                ];

                for (const c of courses) await db.collection('courses').add({ ...c, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
                for (const s of students) await db.collection('students').add({ ...s, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
                for (const s of students) await db.collection('attendance').add({ studentName: s.name, courseName: s.course, date: today, status: Math.random() > 0.2 ? 'present' : 'absent', createdAt: firebase.firestore.FieldValue.serverTimestamp() });
                for (const g of grades) await db.collection('grades').add({ ...g, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
                for (const s of students) await db.collection('fees').add({ studentName: s.name, description: 'Semester 1 Tuition Fee', amount: 450.00, dueDate: '2026-09-30', status: Math.random() > 0.4 ? 'paid' : 'pending', createdAt: firebase.firestore.FieldValue.serverTimestamp() });
        ...
        // START
        // =========================================================
        console.log('🚀 EduPulse (Firebase Rebuild) Loaded!');
    </script>
</body>
</html>`;
...
console.log('✅ Firebase version rebuilt successfully! Fast caching + offline support enabled.');